...
async def all_products(request: Request) -> Response:
    async with aiohttp.ClientSession() as session:
        ...
        favorite_request = functools.partial(session.get, f'{FAVORITE_BASE}/users/1/favorites')
        cart_request = functools.partial(session.get, f'{CART_BASE}/users/1/cart')
        ...
        if products in pending:
            ...
            return web.json_response({'error': 'unable to connect to products service'}, status=504)
        elif products in done and products.exception() is not None:
            [request.cancel() for request in requests]
            logging.exception('Server error connecting to products service', exc_info=products.exception())
            return web.json_response({'error': 'server error connecting to products service'}, status=500)
        else:
...
async def get_products_with_inventory(session: ClientSession, product_response) -> List[Dict]:
    def get_inventory(session: ClientSession,
                      product_id: str) -> Task:
    ...
    for done_task in inventory_done:
        if done_task.exception() is None:
            ...
            logging.exception(
                f'Unable to retrieve product availability information {product_id}',
